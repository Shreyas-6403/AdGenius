...
const LandingPage = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (token) {
          const response = await fetch("https://adgenius-backend-6euo.onrender.com/api/auth/me", {
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://adgenius-backend-6euo.onrender.com${endpoint}`, {
        method: "POST",
        headers: {
